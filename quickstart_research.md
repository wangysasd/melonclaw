# Deep Agents Quickstart：研究 Agent
...
官方示例不是从零手写一个 Agent loop，而是把研究任务交给 `create_deep_agent`：
...
Tools 页面把 `tools=` 的来源分成三类，本项目现在都留出了对应位置：
...
附件中的 Skills 文档把 Skill 定义为一个包含 `SKILL.md` 的目录：前置 YAML metadata 用于启动时发现，正文在任务匹配时才读取，脚本、参考资料和模板等支持文件继续按需加载。这是渐进式披露：常用 Skill 不会把完整流程永久塞进每轮上下文。

本项目新增了 [research-workflow](skills/research-workflow/SKILL.md)，把累计 Agent 已经需要遵守的研究习惯沉淀为可复用流程：拆分研究问题、选择最小工具集合、整理证据、标注推断和不确定性，并保护凭据与临时文件。它的描述特意包含“研究、对比、网页、近期信息”等触发词，因为 Agent 在启动时主要依赖 `description` 判断是否需要激活 Skill。

### 运行文件后端

当前系统使用 `CompositeBackend`。默认后端是本次进程的临时 runtime，`/skills/` 通过独立路由直接指向仓库根目录下与 `src/` 平级的 `skills/`，因此不会在 `temp/melonclaw-*/skills/` 创建副本：
...
本项目默认使用 `.env` 中的 DeepSeek 作为模型、Tavily 作为搜索服务。DeepSeek 仍通过 `ChatOpenAI` 访问其 OpenAI 兼容接口；搜索则由 `TavilyClient.search()` 执行，结果作为普通 callable 的返回值交给 DeepAgents。这样可以直接观察“模型决定调用工具、工具负责外部 API、副作用由工具边界控制”的完整链路。
...
- 本地 Shell 后端：`LocalShellBackend(..., virtual_mode=True)` 让文件工具使用项目根目录 `temp/` 下的临时 runtime，并让 `execute` 在该目录作为工作目录执行；但 Shell 命令本身可以访问宿主机任意路径，因此它只适合受信任的本地学习 CLI，生产 Web 服务不应直接暴露这个后端。文件工具必须使用 `/summary.md` 等虚拟路径，不能把终端中显示的 runtime 宿主路径传回 `write_file` 或 `read_file`。
...
| 本项目审批 | `eval` 自身不审批，但只开放受限能力 | `execute` 与写文件进入 HITL |
...
- `subagents=True`：存在 Deep Agents `task` 工具时，Interpreter 可以用顶层 `task({...})` 编排 `general-purpose` 子 Agent。它与 `tools.*` PTC 是两条不同桥接路径；子 Agent 仍继承累计应用的 `interrupt_on` 配置。

PTC 是明确的权限边界。它的内部调用不会逐次经过父 Agent 的 `interrupt_on` / ToolNode，因此项目不把 `write_file`、`edit_file`、`delete`、`execute` 或 MCP 工具加入白名单。未来增加任何付费、写数据库、发消息、交易或部署工具时，也不能仅因为“主 Agent 已配置 HITL”就把它放进 PTC；应先确认无需逐次审批，或改为直接工具调用。

### 运行与观察

依赖由 `langchain-quickjs==0.3.5` 提供；`deepagents==0.7.5` 的 `quickjs` extra 要求至少 0.3.3。由于 Interpreter 仍是 Beta API，项目把已验证版本精确锁定，升级时应重新执行本节的状态与 PTC 验证：
...
`LocalShellBackend` 支持 `execute`，但当前 Deep Agents 不允许它与覆盖路径的 `FilesystemPermission` 一起使用，因此项目只使用 `core/hitl.py` 中的 `interrupt_on` 审批配置：运行时 `write_file`、`edit_file`、`delete` 和 Shell `execute` 都在真正执行前暂停。它保留了 CLI/Web 的一致行为，但不提供沙箱隔离；任何部署到共享环境前都必须更换为受隔离的 SandboxBackend。
...
- 页面模拟用户来自数据库；一个用户可以属于多个租户，下拉框展示“用户中文名-租户中文名”。租户只是用户标签，切换标签不会切换该用户的 Project 或 Conversation。模拟用户下面先显示“项目”文件夹列表，并在同一行提供“新增项目”；点击文件夹后进入该 Project 的聊天会话列表，再次点击已打开的文件夹即可收起会话，Project 不再作为下拉筛选器。所有按 conversation_id 的接口都按 `user_id + project_id` 再次校验归属，不存在或归属不匹配统一返回 404。它不是认证机制，上线时应由认证上下文产生 user_id。
...
## MCP Server/Client 学习包

附件中的 MCP 文档强调了三个边界：Server 暴露工具，Client 负责发现和调用工具，传输层决定两者如何通信。本项目新增的 `example/mcp/` 用一个数学 Server 把这条链路具体化：
...
项目根目录的 [mcp.json](mcp.json)（与 `src/` 同级）是多个 MCP 服务的配置入口。文件采用常见的 `mcpServers` 顶层键和 `type` 连接类型，便于复制到支持该格式的其他客户端；项目加载器仍兼容旧的 `servers` 和 `transport`。暂时不用 MCP 时，可以将文件设为 `{}` 或把整段 JSON 用 `//` 注释掉，累计 Agent 会按没有 MCP 正常启动。

### Server

`math_stdio_server.py` 和 `math_sse_server.py` 都是可独立阅读和运行的完整 FastMCP Server，各自注册 `add`、`multiply` 和 `explain_transport` 三个工具。两份少量重复代码是有意的：学习任一传输时不需要跳转到共享工厂模块。STDIO Server 名为 `MCP Study Math STDIO`，SSE Server 名为 `MCP Study Math SSE`。STDIO Server 由 Client 启动本地子进程，协议消息走 stdin/stdout，因此不向 stdout 写普通日志：
...
SSE 适合学习和兼容已有服务；对于新的 HTTP 服务，MCP 文档建议优先了解 `streamable-http` 传输。
...
`MultiServerMCPClient.get_tools()` 会把 MCP 工具转换为 LangChain 工具。按照附件文档，它默认是无状态的：每次工具调用会创建新的 MCP ClientSession；如果 Server 需要跨调用保持会话，应改用 `client.session()` 显式管理生命周期。
...
Tushare 官方给出的配置使用常见的 `mcpServers` 顶层键，并把凭据放在远程 URL 中。本项目现在保留这一顶层形状，显式增加 `type: "http"`，加载时再转换成 `MultiServerMCPClient` 使用的 `transport: "http"`。
...
累计 `melonclaw` 应用复用了同一个 `mcp.json` 服务目录，但把应用侧职责放在 `core/mcp_config.py`：
...
本次端到端验证使用问题“请查询上交所 2026-01-05 是否开市，只使用 Tushare 数据”。DeepSeek 自动生成并执行了下面的工具调用：
...
这次验证还暴露了同步/异步边界：若用 `agent.stream()`，LangGraph 会同步调用 MCP `StructuredTool` 并报“不支持同步调用”；因此应用把异步边界提升到 CLI 最外层，只调用一次 `asyncio.run()`，让 Agent 构建、工具发现、模型流和 MCP 工具运行在同一个事件循环中。工具选择、参数生成、远程调用和最终回答由此形成完整闭环。
...
- 远程服务、网络或账户权限异常时，Client 保留异常类型和调用栈用于学习，但其中的 token 会显示为 `<redacted>`。
- 离线测试使用测试专用值，不访问 Tushare；远程冒烟验证失败不会改变配置解析和脱敏测试的有效性。

## Yuxi 用户/组织 Workspace 与 MelonClaw 用户租户标签、Project Memory 设计

研究日期：2026-09-07。本文同时记录设计取舍和当前 MelonClaw 的实现进展。
...
因此，MelonClaw 不应把“组织标签”简单替换成文件路径，也不应把 LangGraph checkpoint 当成长期记忆。建议采用四层模型：
...
| Organization memory | 如果未来引入真实组织共享，保存组织明确发布的共享事实、流程和术语 | 组织成员 | PostgreSQL Memory Store；组织 Workspace 文件作为原始资料 |
| Project/episodic memory | 当前项目结论或历史对话检索 | 当前用户的 Project | Project 作用域记录 + 现有 `chat_messages` |

其中，组织 Memory 是在 Yuxi 经验上为 MelonClaw 增加的能力，不是 Yuxi 当前已有的组织文件 Workspace。这个差异必须写进产品和权限契约，避免误把“组织可以共享资源”理解成“组织可以读取所有用户文件”。
...
- **默认 Project**：MelonClaw 为每个用户维护一个可见的“临时默认”，旧会话和未指定 Project 的新会话都归并进去；Yuxi 手动创建的 selectable Project 可以承载多个 Conversation，但未指定 Project 时会为该 Conversation 创建一个隐藏的 implicit Project，并非用户唯一默认 Project。
...
- **长期 Memory**：Yuxi 有用户级 `agents/MEMORY.md`、显式 `remember_memory`、用户历史检索和写入锁；MelonClaw 当前只有 PostgreSQL Checkpointer、`chat_messages` 和 Project 共享文件，没有 User/Tenant/Project Memory middleware，也没有 Store-backed 跨 thread 记忆。

因此，MelonClaw 当前更准确的描述是“带用户标签上下文和 Project ownership 校验的单部署多用户演示”，而不是完整的企业多租户系统。`tenant_id` 在当前模型中只表示用户标签；真正的业务归属是 `user_id`、`project_id` 和 `conversation_id`。`namespace` 仍只作为 Store/事件/执行树的技术分层，不承担业务授权职责。
...
当前应用已具备 `tenants` 和 `user_tenants` 基础关系：一个用户可以有多个租户标签，`user_tenants.id` 是代理主键，`(user_id, tenant_id)` 只保持关系唯一。请求可以携带一个标签用于展示或运行上下文，但不能用它切分同一用户的 Project、Conversation、Workspace 或 Memory；这些对象的归属分别由 User、Project 和明确的 Memory scope 决定。
...
Agent 看到的是经过 resolver 映射的虚拟路径，例如 `/user-data/` 和 `/project/`，而不是宿主绝对路径。`/skills/` 继续只读。用户 Workspace 只允许用户本人和受控系统流程写入；不能把多个用户 Workspace 通过一个宽泛的根目录交给 `LocalShellBackend`。如果未来增加真实组织共享，其 Workspace 应另建组织成员 ACL，不应复用 tenant 用户标签。

Workspace 文件是资料和可编辑上下文，不等于 Memory。Project Workdir 中的文件不会因为存在就自动进入每轮 prompt；Agent 需要通过显式引用、搜索或工具读取。

#### 4.3 Memory 的事实模型

建议新增一个统一的 `memory_items` 概念，核心字段如下：
...
同时保留 `memory_events` 审计表，记录创建、修改、删除、发布、撤销、操作者、run/request 和前后版本。建议 PostgreSQL 记录是 Memory 的事实源；如果需要 Yuxi 风格的可见文件，可把用户 Memory 渲染为 `agents/MEMORY.md` 投影，但 prompt 和工具读写仍走 Memory Service，不让模型直接编辑该文件来绕过审计和权限。组织 Memory 更应使用结构化记录，避免多人并发编辑一个 Markdown 文件产生难以合并的冲突。

唯一性和所有检索条件都至少包含 `(scope_type, scope_id)`；固定记忆再加 `key` 唯一。用户 Memory 的 key 应为 `(user, user_id, key)`，Project Memory 的 key 应为 `(project, project_id, key)`。tenant 标签可以作为审计或展示快照保存，但不能进入决定对象归属的主键或检索边界。

### 5. DeepAgents 集成方式

保留当前 PostgreSQL Checkpointer 作为短期 thread state；长期 Memory 使用持久化 Store/Memory Repository，不能使用 `InMemoryStore`。如果通过 DeepAgents 文件能力暴露长期存储，可以用 `CompositeBackend` 把默认 `StateBackend` 和 `/memories/` 的 `StoreBackend` 分开：

```text
/summary.md、临时研究文件  -> StateBackend，按 thread 丢弃
/memories/                 -> 持久化 Store，跨 thread 保留
...
- 从 `runtime.context` 读取 user/project/run 身份，每次请求动态计算可见 scope；不在全局 Agent 单例中缓存某个用户的 prompt。
- 仅注入有界的“核心记忆摘要”：用户记忆和当前 Project 摘要分开标记，并明确都是不可信参考资料。
- 暴露受限的异步工具：`search_memory`、`read_memory`、`remember_user_memory`、`remember_project_memory`、`forget_memory`。工具不接受任意 `user_id/project_id/path` 作为可信授权输入，目标由服务器上下文和 ACL 决定。
- 只有当前用户明确要求长期保存时才写入；Project 记忆也必须明确写入当前 Project。历史检索默认只读，不触发写入。

当前 `AgentContext` 已包含用户、租户标签和 Project 展示字段；长期 Memory 版本应补充上面的 scope 权限快照，并给每次执行一个服务端生成的 `run_id`。如果继续复用启动时单例 graph，就必须让 middleware 按 invocation context 动态读取；如果框架版本不适合动态 middleware，则按请求构建 graph，但共享模型和工具目录，不能共享带用户内容的 middleware 实例。
...
认证上下文
  -> 校验 user
  -> 校验可选 tenant 标签属于 user
  -> 校验 conversation 与 project 归属
  -> 生成 run/request identity
  -> 按 ACL 读取 user + project 核心 Memory
  -> 以不可信 <memory_data scope=...> 注入主 Agent
  -> 按需 search/read 历史或 Workspace
  -> 明确请求后调用 remember/forget
  -> scope 锁 + version/CAS + 审计 + 幂等提交
  -> 最后提交业务终态，再发送 SSE completed
```

固定记忆的第一版可以采用 Yuxi 的简单策略：追加去重、精确替换、单条和总量上限、UTF-8 校验、截断标记。历史记忆继续优先使用 PostgreSQL 全文/关键词检索；只有数据规模和效果证明需要时，再引入 pgvector 或 Milvus，并且向量查询必须先带 user/project scope 过滤，不能先全库召回后再猜权限。
...
| project | 当前用户的 Project | 当前用户明确指定当前 Project；默认不升级为用户全局记忆 |
| episodic history | 本人历史；共享历史必须另有 ACL | 由聊天业务流产生，不允许 Memory 工具改写 |

“global”若保留，应解释为“当前部署内、经过明确 ACL 的全局资源”，不能把 tenant 标签当成授权边界。跨用户读取只能由平台级治理操作完成，并留下独立审计记录。

所有写操作还应具备以下约束：

- 事务锁定 Memory scope，而不是只锁 conversation；共享 Project Memory 使用 row lock/advisory lock 加 version/CAS。
...
- Memory 内容按数据处理，不能覆盖当前 system/user 指令；禁止凭据、Token、临时推测和只属于当前 Project 的私密事实进入 user 核心记忆。
- 用户切换 tenant 标签时，Project、Conversation 和 Memory 不能被清空、切换或串联；标签变化只影响展示和当前运行上下文。

### 8. 推荐实施顺序（长期 Memory 仍属于后续设计）

1. **第一阶段：User Memory**。沿用 Yuxi 的显式记住、大小预算、历史按需检索和当前 Run 校验，按 `user_id` 建立长期记忆。
2. **第二阶段：Project Memory**。把当前 Project 的结论与用户全局偏好分开，提供提升/降级的明确动作，防止项目机密污染用户记忆。
3. **第三阶段：真实组织共享**。若确实需要组织或团队共享，再引入独立的 Organization/Member/ACL 模型，不复用 tenant 用户标签。
4. **第四阶段：检索增强**。用真实查询日志评估关键词检索，再决定是否引入 embedding、摘要压缩、过期策略和 Memory 管理页面。

验收重点不是“模型说它记住了”，而是重新读取 PostgreSQL、Memory 事件和对应 Workspace 字节，验证以下负向案例：错误 user_id 读不到 Project/Memory；tenant 标签切换不会改变 Project 集合；未明确要求时不会写；旧 run/错误 user_id 不能写；同一 request 重试不会重复写；删除后旧数据不会被恢复。

### 9. 本次已实现：Project 共享工作目录

本次先落地 Yuxi 的 Project/Conversation/Workdir 关系，长期 User/Project Memory 仍按上面的设计分阶段实现：
...
- 新增 `POST /api/projects`、`GET /api/projects`；创建会话时可传 `project_id`。不传时自动使用当前用户唯一的“临时默认” Project，因此第一次聊天不需要先手动创建 Project。
...
- 数据库升级是幂等的。每个已有用户都会自动拥有一个名为“临时默认”的默认 Project；Project 功能上线前产生的旧会话会全部归并到该用户的默认 Project，不会删除旧消息或 checkpoint。上一版迁移生成的“历史会话”项目会标记为 deleted，原有文件字节不做破坏性删除。

当前边界：Project 仍属于当前用户，尚未实现 Yuxi 那种可选 Project 成员、用户标签之外的租户级共享 Project 或独立的 Store-backed 长期 Memory；当前 `LocalShellBackend` 仍只适合本机学习环境，生产 Web 应替换为带授权边界的 Sandbox/受控 backend。
