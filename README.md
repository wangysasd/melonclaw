...
聊天页面会把工具活动与最终回答分层显示：成功的工具默认收起，失败或没有收到终态的工具会展开并标记原因；流式连接异常时会保留已收到的回答，并提供“重新同步会话”入口。切换会话不会中断原会话的 SSE，原会话会在后台继续执行；切回时会加载最新历史，并继续接收尚未结束的输出。切换会话后如果服务端仍显示上次请求未结束，可点击该按钮强制重新读取会话历史；切换用户或项目仍会中止当前流，切换会话或刷新页面后待审批状态会从服务端恢复。
...
AI 执行开始时，页面会短暂显示“正在工具筛选”；这是本次运行的临时状态，动态选择器的内部 `{"tools": [...]}` 不会展示在聊天正文或写入前端消息历史。
