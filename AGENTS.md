...
- 用户要求新增能力时，应在现有应用上迭代、扩展或重构，保持Web、数据库和 Agent 组装链路的一致性；不要为单个链接或单个功能创建独立项目。
- 新增功能完成后，更新根目录 [note.md](note.md)。记录功能背景、问题定义、方案、关键取舍、实现位置、验证方式、可观察点和当前边界。内容应使用让人容易理解的语气。
