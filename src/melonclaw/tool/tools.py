...
def build_mcp_catalog_tool(
    ...
    return StructuredTool.from_function(
        ...
        description=(
            "只读查询当前运行时已加载的 MCP 服务、传输方式和工具名称。"
...
) -> tuple[list[ToolDefinition], dict[str, tuple[str, ...]]]:
...
async def load_mcp_tools(
    ...
    try:
        tools, _ = await _load_mcp_tools_with_catalog(servers, tool_allowlists)
        return tools
...
async def build_agent_tools(settings: Settings) -> list[ToolDefinition]:
    ...
    try:
        mcp_tools, catalog = await _load_mcp_tools_with_catalog(
    ...
    tools.append(build_mcp_catalog_tool(settings.mcp_servers, catalog))
    return tools
